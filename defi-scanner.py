...
PATTERNS = {
    ...
    34: {"name":"Sandwich Attack Surface","severity":"MEDIUM","regex":[r'minOut.*0|amountOutMin.*0|deadline.*block'],"keyword":["slippage","minOut","deadline","amountOutMin"],"description":"No/weak slippage protection enables sandwich attacks","fix":"Require minOut with slippage tolerance"},
    35: {"name":"NFT Reentrancy via Callbacks","severity":"HIGH","regex":[r'safeTransferFrom.*![nonReentrant]'],"keyword":["onERC721Received","ERC721","safeTransferFrom"],"description":"NFT callback can trigger reentrancy if no guard","fix":"Use ReentrancyGuard or update state before safeTransferFrom"},
    36: {"name":"L2 Sequencer Downtime","severity":"MEDIUM","regex":[],"keyword":["sequencer","L2","optimism","arbitrum","base"],"description":"Oracle stale during L2 sequencer downtime","fix":"Check sequencer uptime feed before using oracle"},
...
class DeFiScanner:
    ...
    def generate_report(self):
        ...
        lines.append(f"\n{'=' * 60}")
        lines.append(f"Scanned with 26 detection rules (covering 26/50 DeFi patterns)")
        return '\n'.join(lines)
